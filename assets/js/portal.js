(function(){
  ...
  function authGate(){
    ...
    const gate=document.createElement('div');gate.id='mneAuthGate';gate.className='auth-gate';gate.innerHTML=`<div class="auth-shell"><div class="auth-brand"><img src="assets/images/mce-logo.png" alt="Mastering NDA/NA English"><div><b>Mastering NDA/NA English</b><span>Student Access</span></div></div><div class="auth-visitor"><span>Visitors till now</span><strong data-visitor-count>—</strong></div><div class="auth-tabs"><button class="active" data-auth-tab="register">REGISTER</button><button data-auth-tab="login">LOGIN</button></div><section data-auth-panel="register"><h2>Register once. Access everything.</h2><p>Registration is required for online tests and downloadable PDFs. No OTP or email-link verification is used.</p><form id="mneRegisterForm" class="auth-form"><label>Name<input name="fullName" required autocomplete="name" maxlength="100"></label><label>Class<select name="className" required><option value="">Select Class</option>${cls}</select></label><label>State / UT<div class="search-select" id="mneStateSelect"><div class="search-select-control"><input name="state" required placeholder="Search or choose State/UT" autocomplete="off" role="combobox" aria-autocomplete="list" aria-expanded="false" aria-controls="mneStateMenu"><button class="search-select-toggle" type="button" aria-label="Show all States and Union Territories">⌄</button></div><div class="search-select-menu" id="mneStateMenu" role="listbox"></div></div><small class="field-help">Tap the field to see all options, or type a few letters to search.</small></label><label>PIN Code<input name="pincode" inputmode="numeric" pattern="[0-9]{6}" maxlength="6" required placeholder="6-digit PIN"></label><label class="auth-full">School<input name="schoolName" id="mneSchoolName" maxlength="180" required placeholder="School name"></label><label class="auth-check auth-full"><input type="checkbox" name="schoolNone" id="mneSchoolNone"> <span>No school / None</span></label><label class="auth-full">Email<input name="email" type="email" required autocomplete="email" placeholder="name@example.com"></label><div class="auth-message auth-full" id="mneRegisterMessage"></div><button class="btn btn-primary auth-full" type="submit">Register & Continue</button></form></section><section data-auth-panel="login" class="hidden"><h2>Already registered?</h2><p>Enter the same email used during registration. Unregistered emails cannot enter.</p><form id="mneLoginForm" class="auth-form"><label class="auth-full">Registered Email<input name="email" type="email" required autocomplete="email" placeholder="name@example.com"></label><div class="auth-message auth-full" id="mneLoginMessage"></div><button class="btn btn-primary auth-full" type="submit">Login</button></form></section><p class="auth-note">Email is used as your account identifier. Because OTP verification is intentionally disabled, the system can validate registration and email format but cannot independently prove ownership of the email address.</p></div>`;document.body.appendChild(gate);
  ...
  function unlock(){document.documentElement.classList.remove('auth-pending');document.getElementById('mneAuthGate')?.remove();document.body.classList.add('student-authenticated');addAccountChip()}
  function addAccountChip(){if(document.getElementById('mneAccountChip'))return;const p=profile();if(!p)return;const chip=document.createElement('div');chip.id='mneAccountChip';chip.className='account-chip';chip.innerHTML=`<a href="dashboard.html"><span>${esc(p.name||'Student')}</span><small>Student Hub</small></a><button type="button" title="Logout">Logout</button>`;document.body.appendChild(chip);chip.querySelector('button').addEventListener('click',async()=>{try{await call('logout',{},true)}catch{}clearSession();location.href='index.html'})}
