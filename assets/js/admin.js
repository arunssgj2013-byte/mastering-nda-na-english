(function(){
  ...
  function renderSummary(s){
    const cards=[['Total Visitors',s.total_visitors||0],['Total Visits',s.total_visits||0],['Registered Users',s.registeredUsers??s.registeredStudents??0],['Active Users (30d)',s.activeUsers30d??s.activeStudents30d??0],['Quiz Attempts',s.totalQuizAttempts||0],['Average Accuracy',pct(s.averageAccuracy||0)]];
  ...
  function fillFilters(students){
    const cls=[...new Set(students.filter(x=>(x.user_type||'STUDENT')==='STUDENT').map(x=>x.class_name).filter(Boolean))].sort();const states=[...new Set(students.map(x=>x.state_ut).filter(Boolean))].sort();
  ...
  function renderStudents(){
    const all=dashboardData?.students||[],q=String($('#adminSearch')?.value||'').trim().toLowerCase(),role=$('#adminRoleFilter')?.value||'',cl=$('#adminClassFilter')?.value||'',st=$('#adminStateFilter')?.value||'';
    const rows=all.filter(x=>{const type=x.user_type||'STUDENT',hay=[x.full_name,x.email,x.school,x.state_ut,x.pincode,type].join(' ').toLowerCase();return(!q||hay.includes(q))&&(!role||type===role)&&(!cl||x.class_name===cl)&&(!st||x.state_ut===st)});
    $('#adminStudentCountLabel').textContent=`Showing ${rows.length} of ${all.length} users`;
    $('#adminStudentsBody').innerHTML=rows.length?rows.map(x=>{const [label,tone]=statusLabel(x.growth_points),type=x.user_type||'STUDENT',roleLabel=type==='TEACHER'?'Teacher':type==='OTHER'?'Other User':'Student';return `<tr><td><b>${esc(x.full_name)}</b><small>${esc(x.email)}</small></td><td>${esc(roleLabel)}</td><td>${type==='STUDENT'?esc(x.class_name||'—'):'—'}</td><td>${esc(x.state_ut||'—')}</td><td>${esc(x.pincode||'—')}</td><td>${esc(x.school||'—')}</td><td>${fmtNum(x.total_attempts)}</td><td>${Number(x.best_score||0).toFixed(2)}</td><td>${pct(x.average_accuracy)}</td><td><span class="perf-pill ${tone}">${esc(label)} ${Number(x.growth_points||0)>0?'+':''}${Number(x.growth_points||0).toFixed(1)} pts</span></td><td>${fmtDate(x.last_activity_at||x.latest_attempt_at||x.last_login_at)}</td><td><button class="table-action" data-student-id="${esc(x.student_id)}">View</button></td></tr>`}).join(''):'<tr><td colspan="12" class="empty-cell">No users match these filters.</td></tr>';
  ...
  function renderRecent(){
    ...
    $('#recentAdminActivity').innerHTML=acts.slice(0,12).map(x=>`<div class="admin-feed-row"><span>${esc(x.event_type)}</span><div><b>${esc(x.full_name||x.email||'User')}</b><small>${esc(x.resource_title||x.resource_id||'')} · ${fmtDate(x.created_at)}</small></div></div>`).join('')||'<p class="muted">No activity records yet.</p>';
  ...
  async function openStudent(studentId){
    const modal=$('#studentDetailModal');modal.classList.remove('hidden');$('#studentDetailContent').innerHTML='<div class="admin-detail-loading">Loading user record…</div>';
    try{
  ...
  function exportCsv(){
    const rows=dashboardData?.students||[];if(!rows.length){mark('No user records to export.','error');return}
    const headers=['Name','Email','User Type','Class','State/UT','PIN Code','School','Registered At','Last Login','Login Count','Total Attempts','Unique Papers','Average Accuracy','Best Score','First Accuracy','Latest Accuracy','Growth Points','Last Activity'];
    const val=s=>'"'+String(s??'').replace(/"/g,'""')+'"';
    const lines=[headers.map(val).join(',')].concat(rows.map(x=>[x.full_name,x.email,x.user_type||'STUDENT',x.class_name,x.state_ut,x.pincode,x.school,x.registered_at,x.last_login_at,x.login_count,x.total_attempts,x.unique_papers_attempted,x.average_accuracy,x.best_score,x.first_accuracy,x.latest_accuracy,x.growth_points,x.last_activity_at].map(val).join(',')));
    const blob=new Blob(['\ufeff'+lines.join('\r\n')],{type:'text/csv;charset=utf-8'}),url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download='Mastering-NDA-User-Performance-'+new Date().toISOString().slice(0,10)+'.csv';a.click();setTimeout(()=>URL.revokeObjectURL(url),1000);mark('User performance CSV downloaded.','ok');
  }
  ...
  function bind(){
    ...
    ['#adminSearch','#adminRoleFilter','#adminClassFilter','#adminStateFilter'].forEach(sel=>$(sel)?.addEventListener(sel==='#adminSearch'?'input':'change',renderStudents));
